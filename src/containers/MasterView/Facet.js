...
const FacetListItem = ({item, onChange}) => {
  ...
  return (
    <FacetCheckbox label={item.value} checked={item.checked} count={item.count} onClick={onClick} />
  );
};
...
const FacetList = ({list, onChange}) => (
  <div className="kgs-facet-list">
    {list.map(item => (
      <FacetListItem key={item.id} item={item} onChange={onChange} />
    ))}
  </div>
);
  ...
  case "list":
    ...
    parameters = {
      list: facet.keywords.map(keyword => ({
        value: keyword.value,
        count: keyword.count,
        checked: Array.isArray(facet.facet.value)?facet.facet.value.includes(keyword.value):false
      })),
  ...
  if (Component) {
    return (
      <div className="kgs-facet">
        <div className="kgs-facet-title">{facet.facet.fieldLabel}</div>
