...
const ShapeFilterBase = ({type: {type, label, count, active, disabled}, onClick}) => (
  <div className={`kgs-fieldsFilter__shape${active?" is-active":""}${disabled?" is-disabled":""}`}>
    <button onClick={onClick} className="kgs-fieldsFilter__button" disabled={disabled || active}>
      <div>
        <div className="kgs-fieldsFilter__icon">
          <ShapeIcon label={label} shape={type} active={active} />
        </div>
...
class ShapeFilter extends React.Component {
   ...
   render() {
     return (
       <ShapeFilterBase type={this.props.type} onClick={this.onClick} />
     );
   }
}
  ...
  render() {
    ...
    return (
      <div className="kgs-fieldsFilter">
        {types.map(type =>
          <ShapeFilter type={type} key={type.type} onClick={onClick} />
        )}
      </div>
    );
  }
...
export const ShapesFilterPanel = connect(
  ...
  //{itemKey, label, count, active, disabled, onClick}
