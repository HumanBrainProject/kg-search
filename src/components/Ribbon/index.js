...
const RibbonComponent = ({text, counter, suffix, icon}) => (
...
export function Ribbon({data, mapping}) {
  if (!mapping || !data) {
    return null;
  }
  ...
  return (
    <RibbonComponent text={mapping.content} counter={counter} suffix={suffix} icon={mapping.icon} />
  );
}