...
public class ContributorMerger extends Merger<Contributor> {
    ...
    protected void merge(Contributor parent, Contributor child) {
        ...
        if (child.getDatasetContributions() != null) {
            ...
            parent.getDatasetContributions().addAll(child.getDatasetContributions().stream().filter(d -> parentContributionsWithOld.contains(d.getReference())).sorted().collect(Collectors.toList()));
        }
        if (child.getCustodianOfDataset() != null) {
            ...
            parent.getCustodianOfDataset().addAll(child.getCustodianOfDataset().stream().filter(d -> !parentCustodiansWithOld.contains(d.getReference())).sorted().collect(Collectors.toList()));
        }
        if (child.getCustodianOfModel() != null) {
            ...
            final Set<String> parentCustodiansWithOld = Stream.concat(parentCustodians.stream(), parentCustodians.stream().map(p -> String.format("Person/%s", p))).collect(Collectors.toSet());
            parent.getCustodianOfModel().addAll(child.getCustodianOfModel().stream().filter(d -> !parentCustodiansWithOld.contains(d.getReference())).sorted().collect(Collectors.toList()));
        }
        if (child.getModelContributions() != null) {
            ...
            final Set<String> parentContributionsWithOld = Stream.concat(parentContributions.stream(), parentContributions.stream().map(p -> String.format("Person/%s", p))).collect(Collectors.toSet());
            parent.getModelContributions().addAll(child.getModelContributions().stream().filter(d -> !parentContributionsWithOld.contains(d.getReference())).sorted().collect(Collectors.toList()));
        }
    }
}
