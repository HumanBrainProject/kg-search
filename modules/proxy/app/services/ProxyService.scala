...
class ProxyService @Inject()(wSClient: WSClient)(implicit executionContext: ExecutionContext) {
  ...
  def queryIndex(esIndex: String,
                             proxyUrl: String,
                             es_host: String,
                             transformInputFunc: ByteString => ByteString = identity
                            )(implicit request: Request[RawBuffer], executionContext: ExecutionContext): (WSRequest, Map[String, String]) = {
  ...
  def modifyQuery(newUrl: String, indexHint: String)(implicit request: Request[RawBuffer]): WSRequest = {
    wSClient.url(newUrl) // set the proxy path
